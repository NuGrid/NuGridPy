    ...
    def elemental_abund(self,cycle,zrange=[1,15],ylim=[-12,5],title_items=None,
        ...
        if plotType=='PPN':
            ...
            if z_pin!=None:
                ...
                if pin!=None:
                    ...
                    for z_i in z_el[zmin_ind:zmax_ind]:
                        ...
                        zpinidx=where(obs_file.data['Z']==str(z_i))[0]
                        if len(zpinidx)==0:
                            el_abu_obs.append(0)
                        elif len(zpinidx)>1:
                            tmp=obs_file.data['[X/Fe]'][zpinidx].astype(np.float)
                            tmp=np.sum(tmp)
                            el_abu_obs.append(tmp)
                        else:
                            el_abu_obs.append(obs_file.data['[X/Fe]'][zpinidx][0].astype(np.float))
                    el_abu_pin=np.power(10,el_abu_obs)
                elif ref==-2:
            ...
            pl.plot(z_el[zmin_ind:zmax_ind],np.log10(el_abu)+offset,label='Simulations',\
                   marker='o',linestyle='--')#np.log10(el_abu))#,**kwargs)
            j=0        # add labels
            for z in z_el[zmin_ind:zmax_ind]:
                pl.text(z+0.15,log10(el_abu[j])+offset+0.05,el_name[j])
                j += 1
            if pin_filename!=None:
                pl.plot(z_el[zmin_ind:zmax_ind],el_abu_obs,label='Observations',marker='*',linestyle=':')
